...
system = """You are an assistant for question-answering tasks.
    ...
    And also, you really must show all URLs and titles that you have used for reference, below the answer."""
...
def retrieve(state):
    ...
    return {"documents": documents, "question": question, "search_count": 0, "hallucination_check_count": 0}
...
def generate(state):
    ...
    for doc in documents:
        pprint(doc)
        references.append({"url": doc.metadata["source"], "title": doc.metadata["title"]})
    ...
    print("=====GENERATED=====")
    print(generation)
    return {"documents": documents, "question": question, "generation": generation, "references": references}
...
def grade_generation_v_documents_and_question(state):
    ...
    if state["hallucination_check_count"] > 1:
